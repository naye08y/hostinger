...
if ($id == '' || $token == '') {
    ...
    if ($token == $token_tmp) {
        ...
        if ($sql->fetchColumn() > 0) {
            $sql = $con->prepare("SELECT nombre, ruta_img, descripcion, precio1, precio2, precio3 FROM productos WHERE id=?  AND categoria_id=3 LIMIT 1");
            $sql->execute([$id]);
...
<head>
    ...
    <title>Detalles de productos Nutricionales</title>
